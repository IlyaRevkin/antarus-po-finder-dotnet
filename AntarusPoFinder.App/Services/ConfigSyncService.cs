...
public static class ConfigSyncService
{
    ...
    private static readonly HashSet<string> SkipSettingsKeys = new()
    {
        ...
        // ОТСУТСТВУЮТ — они синхронизируются, ровно как политика единого входа выше. По замыслу
        // владельца администратор вводит их ОДИН раз, после чего они доезжают до всех машин, которым
        // положено выкладывать инструкции на хостинг: иначе выложить инструкцию мог бы только тот, кто
        // вписал ключ у себя, а «это не дело». Доступ на запись в бакет предприятия — осознанное
        // решение владельца, синхронизировать его МОЖНО (инструмент внутренний).
        ...
        // Дословно от Ильи: «настройки принтера и сохранение их для этикетки это полностью не
        // синхрон параметры, так как для каждой машины они уникальны, кроме дизайна и текста».
