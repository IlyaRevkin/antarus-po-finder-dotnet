...
public static class ConfigSyncService
{
    ...
    {
        // third_disk_path — рядом со вторым диском и по той же причине: буква сетевого диска у
        // каждой машины своя. А вот «дублировать ли инструкцию копией рядом с прошивкой»
        // (instruction_duplicate_on_first_disk) здесь СОЗНАТЕЛЬНО отсутствует — оно синхронизируется.
        // Прежняя настройка third_disk_shortcuts была локальной потому, что ярлык клали ради коллег
        // со старым клиентом — то есть решение зависело от окружения конкретной машины. Дублирование
        // копией — не про окружение, а про то, как предприятие хранит документы («инструкция лежит и
        // на диске с прошивками, и на хостинге»), и разъехавшись по машинам, оно дало бы диск, где
        // половина папок с документом, а половина пустая. Сам ключ third_disk_shortcuts больше не
        // читается никем и остаётся в списке только чтобы не уехать в общий конфиг со старых машин.
