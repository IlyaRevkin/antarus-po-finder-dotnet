...
public static class ConfigSyncService
{
    ...
    private static readonly HashSet<string> SkipSettingsKeys = new()
    {
        ...
        // Ключи доступа к хранилищу на хостинге — это доступ на ЗАПИСЬ во весь бакет, а не
        // настройка: уехав в общий конфиг, они оказались бы на каждой машине предприятия, включая
        // те, что вообще ничего не выкладывают. Адрес/бакет/регион/префикс и сам выключатель выкладки
        // при этом синхронизируются (их здесь нет) — это одинаковая для всех политика хранения,
        // которую администратор задаёт один раз.
