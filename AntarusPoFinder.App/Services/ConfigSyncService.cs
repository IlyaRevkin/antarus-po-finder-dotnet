...
public static class ConfigSyncService
{
    ...
    /// дисков), и администратор задаёт их ОДИН раз, после чего они доезжают до
    ...
    {
        // Функция третьего диска убрана (инструкции теперь только на первом диске + хостинг), но его
        // ключи third_disk_path/third_disk_shortcuts/instruction_duplicate_on_first_disk остаются в
        // списке пропуска: если у кого-то на старом клиенте они ещё лежат в базе, пусть не уезжают в
        // общий конфиг и не будят их у других. Никто их больше не читает.
        "root_path", "second_disk_path", "third_disk_path", "third_disk_shortcuts",
        "instruction_duplicate_on_first_disk",
        // Ключи доступа к хранилищу на хостинге (s3_access_key/s3_secret_key) ЗДЕСЬ СОЗНАТЕЛЬНО
        // ОТСУТСТВУЮТ — они синхронизируются, ровно как политика единого входа выше. Администратор
        // вводит их ОДИН раз, после чего они доезжают до всех машин, которым положено выкладывать
        // инструкции на хостинг: иначе выложить инструкцию мог бы только тот, кто вписал ключ у
        // себя, а это никуда не годится. Доступ на запись в бакет предприятия — осознанное решение,
        // синхронизировать его МОЖНО (инструмент внутренний).
        //   • Secret Access Key едет в снимке ЗАШИФРОВАННЫМ (см. ConfigService.S3SecretKey — префикс
        //     "enc:" поверх ConfigFileCrypto): в БД он и так лежит шифротекстом, а GetAllSettings
        //     отдаёт значение как есть, поэтому в общий конфиг попадает та же строка "enc:…". Ключ
        //     шифрования единый для всех машин (ConfigFileCrypto, не DPAPI), так что любая машина
        //     расшифрует то, что зашифровала другая, — а сам общий конфиг вдобавок ещё раз шифруется
        //     целиком при записи на шару.
        //   • Access Key ID — не секрет (это лишь идентификатор пары), уезжает как обычная строка.
        //   • Совместимость со старым клиентом держится механикой IsSetting: у клиента, где эти ключи
        //     всё ещё в его SkipSettingsKeys, они просто не применятся из общего конфига и не попадут в
        ...
        // ЭТИКЕТКА: железо и подгонка — своё у каждой машины, оформление и тексты — общие.
        // Настройки принтера и сохранение их для этикетки — совсем не синхронизируемые параметры:
        // для каждой машины они уникальны. Общими остаются только оформление и тексты.
        // Своим считается всё, что описывает конкретный принтер и конкретную наклейку в нём: имя
        // принтера, размер наклейки, поля по сторонам, сдвиг протяжки, поворот содержимого под
        // рулон — то есть ровно то, что подставляет кнопка «Считать с драйвера принтера»
        // (PrinterPageFit). Приехав с чужой машины, любая из этих величин описывает чужое железо и
        // печатает мимо наклейки.
        //   • Совместимость со старым клиентом держится тем же приёмом, что у политики единого входа
        //     выше (механика IsSetting): клиент прежней версии эти ключи по-прежнему экспортирует, а
        //     мы их на приёме просто не применяем и в свой экспорт не пишем. Обмен от этого не
        //     ломается — ни в одну, ни в другую сторону.
        //   • label_margin_mm — прежнее единое поле. Ключ мёртвый в том же смысле, что third_disk_path
        //     наверху: его ещё пишет LabelLayout.SaveTo для старых версий программы, но читается он
        //     только там, где нет полей по сторонам. Синхронизировать его тем более незачем.
        // Общими остаются оформление и тексты: вид и положение кода, кегли, подпись назначения с её
        // положением и выравниванием, своя строка, подпись в центре кода, рамка, печать ссылки.
        "label_printer",
