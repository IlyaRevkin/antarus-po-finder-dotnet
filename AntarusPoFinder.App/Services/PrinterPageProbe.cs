...
///
/// Читается ИМЕННО пользовательский тикет очереди (<see cref="PrintQueue.UserPrintTicket"/>): это те
/// самые настройки, которые человек выставил в свойствах принтера — размер наклейки, ориентация,
/// лоток. Тикет по умолчанию (<see cref="PrintQueue.DefaultPrintTicket"/>) — заводской, и брать его
/// первым значило бы вернуть настройки, от которых как раз и уходили.
///
...
public static class PrinterPageProbe
{
    ...
    /// части драйверов бросает исключение прямо здесь, поэтому оно отдельно и с запасным путём.</summary>
