...
public interface IAppHost
{
    ...
    /// <summary>Обменяться тикетами с хранилищем на хостинге (TicketStorageSync): отдать свои
    /// изменения и забрать чужие, в том числе закрытие тикета, сделанное вне офисной сети.
    ///
    /// Живёт в оболочке, а не в странице «Тикеты», по двум причинам: обмен идёт и фоном по тому же
    /// таймеру, что остальная синхронизация (страница может быть не открыта вовсе), и у него ОДИН
    /// замок на приложение — два одновременных прохода наперегонки выкладывали бы каждый своё
    /// состояние. Никогда не бросает: недоступное/ненастроенное хранилище — штатное состояние.</summary>
    /// <param name="force">true — человек попросил (открыл страницу, нажал «Обновить»): идти в
    /// хранилище сейчас же. false — фоновый тик, который соблюдает паузу между походами.</param>
    Task SyncTicketsWithStorageAsync(bool force = false);
}
