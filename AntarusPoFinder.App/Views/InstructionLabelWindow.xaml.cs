...
public partial class InstructionLabelWindow : Window
{
    ...
    public InstructionLabelWindow(AppServices services, IAppHost host, string title, string subtitle, string? instructionFile)
    {
        ...
        // Что именно откроется по коду — говорим прямо, и по-разному для разных поводов: обещание
        // «когда документ допишут» на шкафу, которому инструкции не положено, было бы ожиданием,
        // которое никогда не сбудется (см. StubKind).
        LinkText.Text = InstructionStub.KindOf(instructionFile) switch
        {
            StubKind.InDevelopment =>
                ...
                + explanation,
            StubKind.NotPlanned =>
                "На этот шкаф инструкции не выпускается — по ссылке откроется страница с телефоном сервисной службы. "
                + "Страница общая для всех таких шкафов, ссылка постоянная.\n"
                + explanation,
            _ => explanation,
        };
