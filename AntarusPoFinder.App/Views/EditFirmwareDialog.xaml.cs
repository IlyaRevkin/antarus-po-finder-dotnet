...
public partial class EditFirmwareDialog : Window
{
    ...
    /// <summary>Слова, без которых прошивку не показывают в выдаче (см. FwOnDemandTerms).</summary>
    public string ResultOnDemandTerms { get; private set; } = "";
    ...
    private void ConfigureFirmwareFileFields()
    {
        ...
        // Отсечка ТОЛЬКО по исходнику: без .psl собирать не из чего. Условия «и .lfs ещё нет» здесь
        // больше нет — именно оно и съедало кнопку «Пересобрать LFS из PSL», хотя её сделали
    ...
    private void AddExtraFileRow(ExtraFileRow row)
    {
        ...
        var removeBtn = new Button
        {
            Content = "✕",
            Style = (Style)FindResource("SecondaryButton"),
            // Подсказка называет ПОСЛЕДСТВИЕ, а не действие. Прежняя («убрать материал от этой
            // версии») звучала как снятие привязки, и было непонятно, что файл при этом уходит и с
            // диска — а он уходит, если больше ни у кого не числится (см.
            // FirmwareExtraFilesService.DeleteFileIfUnused). Из-за этого поштучное удаление файлов
            // считали отсутствующим и просили завести заново.
            ToolTip = isNew
                ? "Не добавлять этот файл"
                : "Убрать этот материал от версии. Файл удалится и с диска, если он больше нигде не числится. Применяется при сохранении.",
        };
        removeBtn.Click += (_, _) =>
        {
            row.Removed = true;
    ...
    /// <summary>«Удалить инструкцию» — то, чего до сих пор не было вовсе: очистка поля
