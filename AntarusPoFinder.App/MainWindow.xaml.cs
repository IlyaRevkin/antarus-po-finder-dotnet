...
public partial class MainWindow : Window
{
    ...
    private static readonly Dictionary<string, (string Title, string Body)> NavStepText = new()
    {
        ...
        ["newversions"] = ("Модерация прошивок", "Загруженные версии ждут здесь разметки тегами/описанием и подтверждения релиза (прошивка всё равно отображается при поиске)."),
        ...
        ["network"] = ("Сетевые диски","Путь к диску и папка осмотра — настраивается на каждом компьютере отдельно. Справочник оборудования подтягивается и отправляется на общий диск сам, без ручного интервала."),
    ...
    /// <summary>«Сформировать паспорт» — единственная точка входа в работу с шаблоном паспорта
    /// (окном, по той же причине, что и наклейки выше). Есть ещё дубль в Настройки → Печать для
    /// администратора, который там же и загружает шаблоны, но в основном меню кнопка одна.</summary>
    ...
    private void ShowOnboarding(bool markAsShown)
    {
        ...
        steps.Add(new OnboardingStep(() => SettingsNavButton, "Настройки",
            "Доступны любой роли, но по-разному: наладчику и программисту видна только вкладка «Быстрый доступ», администратору — иерархия шкафов, теги, резервация номеров и модерация полностью."));
        if (_vm.SettingsVisible)
        {
            steps.Add(new OnboardingStep(
                () => NavigateAndResolve("settings", () => (_vm.CurrentPageContent as SettingsView)?.OnboardingTarget("tabbar")),
                "Вкладки настроек",
                "Общие, Иерархия, Прошивки, Модерация, Резервация номеров, Теги, Быстрый доступ, Пользователи — у каждой вкладки свой набор действий (администратору — все, остальным ролям — только «Быстрый доступ»)."));
        }
        ...
        // Тема и цвет переехали в Настройки → Общие → «Оформление», поэтому шаг обучения указывает
        // туда же: показывать стрелку на элемент, которого в сайдбаре больше нет, нельзя — обучение
        // упало бы на пустом месте.
