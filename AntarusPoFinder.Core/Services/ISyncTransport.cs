...
public class SyncChangeEntry
{
    ...
    /// <summary>Ревизия маркера, в которой запись появилась. Нужна принимающей машине, чтобы показать
    /// ТОЛЬКО то, что для неё действительно новое: журнал в маркере хранит последние N записей целиком
    /// (MaxChangelogEntries), и без этого поля плашка «Поступили изменения» перечисляла весь журнал
    /// заново на каждом тике — отсюда была жалоба «пишет пять раз одно и то же и ещё 45».
    /// 0 — запись из маркера, написанного версией приложения без этого поля (возраст неизвестен).</summary>
