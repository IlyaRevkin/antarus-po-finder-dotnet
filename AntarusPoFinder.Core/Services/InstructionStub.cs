...
/// (<see cref="StubKind.ServiceNote"/>) имя своё и постоянное — <see cref="NoteFileName"/>, — но
/// сейчас она отдельным файлом НЕ кладётся: вшивается последней страницей в сам PDF при выкладке
/// (<see cref="ServicePageStitcher"/>). Имя осталось, чтобы узнавать и убирать спутники, которые
/// успели лечь на диск по прежнему решению.
///
...
public static class InstructionStub
{
    ...
    /// <summary>Имя страницы-дополнения к готовой инструкции.
    ///
    /// ⚠️ Файлом рядом с документом она БОЛЬШЕ НЕ КЛАДЁТСЯ — решение Ильи: «страницу по сервису, если
    /// есть инструкция, надо вшивать в тот же файл последней». Заказчик открывает по QR один
    /// документ и файла-спутника в сетевой папке не увидит никогда, а на шаре это ещё и сотни
    /// одинаковых файлов. Имя оставлено, чтобы узнавать и убирать спутники, легшие по прежнему
    /// решению (<see cref="RemoveFrom"/>).</summary>
