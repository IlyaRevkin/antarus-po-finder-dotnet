...
public static class LongOperationRules
{
    ...
    /// <summary>Строка про отмену, которая показывается ВСЕГДА, а не только при опасности. Окно хода
    /// больше не модальное: от него уходят в поиск и возвращаются через минуту, и «можно ли жать
    /// „Остановить“» человек должен прочитать, а не вспоминать.</summary>
    public static string CancelHint(LongOperationKind kind, bool formatsController) =>
        CancelWarning(kind, formatsController)
        ?? (kind == LongOperationKind.PlcDeploy
            ? "Остановить можно в любой момент: контроллер останется с той прошивкой, что была в нём до начала."
            : "Остановить можно в любой момент: на диске ничего не изменится, недособранный LFS никуда не поедет.");
