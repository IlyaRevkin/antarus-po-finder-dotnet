...
/// и описание.
///
/// Как стало (речь только про САМ ФАЙЛ .par — проприетарную выгрузку конфигуратора вендора; история
/// правок ведётся не по нему, а по таблице параметров рядом с ним, см. Domain/ParamTable.cs:
/// ревизии, «зачем» от автора и разбор изменений):
...
public static class ParamFileUploadService
{
    ...
    /// при перезаливке». Редакции никуда не удаляются, просто лежат в одной подписанной папке, а не
    /// вперемешку с актуальным файлом.</summary>
