...
public static class DocFileResolver
{
    ...
    /// третий диск инструкцию, заглушка «Инструкция в разработке» и служебный мусор файловой системы.
    ///
    /// Заглушка — принципиально: её смысл в том, чтобы человек в проводнике видел «документа ещё
    /// нет», и если бы программа считала её документом, признак «инструкция ✓» на карточке, печать и
    /// ссылка в QR начали бы врать (см. <see cref="InstructionStub"/>).
    ///
    /// Мусор добавлен по жалобе: «есть прошивка, а инструкции нет, и он почему-то сделал ссылку на
    /// Thumbs.db вместо заглушки». В папке «Инструкция» лежал ровно один файл — `Thumbs.db`,
    /// созданный проводником при просмотре эскизов, — и он проходил как «первый файл, который не
    /// ярлык и не заглушка». Дальше врало всё сразу: карточка загоралась «инструкция ✓», заглушка не
    /// клалась (папка ведь не пустая), а под QR уходила ссылка на служебный файл Windows — с ней
    /// наклейку и наклеили на шкаф. См. <see cref="JunkFiles"/>.</summary>
    public static bool IsNotADocument(string path) =>
        IsShortcut(path) || InstructionStub.IsStub(path) || JunkFiles.IsJunk(path);
    ...
    ///
    /// Ярлыки .lnk и заглушки пропускаются: ярлык-пережиток прежних версий (указатель на документ) —
