...
public class ConfigService
{
    ...
    private static readonly Dictionary<string, string> Defaults = new()
    {
        ...
        ["label_rotation"] = "None",
        // ОФОРМЛЕНИЕ этикетки (сторона QR, кегли, положение кода и подписи, рамка, печать ссылки,
        // тексты) предустановок ЗДЕСЬ НЕ ИМЕЕТ СОЗНАТЕЛЬНО — единственный их источник это
        // LabelLayout.Default, откуда их берёт LabelLayout.FromConfig. Раньше часть из них стояла и
        // тут, и там: пока значения совпадали, лишнее место хранения было незаметно, а стоило
        // поменять умолчание в одном из двух — «Вернуть исходный макет» и чистая машина стали бы
        // давать разные этикетки. Пустая строка отсюда для LabelFlag/LabelNumber означает ровно
        // «не настроено», и запасное значение берётся из записи макета.
    ...
    /// <summary>Пометить настройки как «сохранил человек здесь и сейчас». Приём общего конфига не
    /// затирает такую настройку значением из снимка, который старше правки (см. таблицу
    /// settings_local_edits и ConfigSyncService.ApplyToDatabase).
    ///
    /// Отмечать нужно ТОЛЬКО то, что человек сохранил осознанно и явно, — не всё подряд, что
    /// проходит через <see cref="Set"/>. Иначе служебная запись (watermark синхронизации, счётчик,
    /// «последний выбранный бланк») притворилась бы правкой человека и начала бы отбивать общий
    /// конфиг, который в этом ключе как раз и должен побеждать.</summary>
    public void MarkEditedHere(IEnumerable<string> keys) => _db.MarkSettingsEditedLocally(keys, DateTime.Now);
