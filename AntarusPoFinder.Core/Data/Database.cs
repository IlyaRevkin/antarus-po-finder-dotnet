...
public partial class Database : IDisposable
{
    ...
    private void Migrate()
    {
        Exec("""
             ...
             -- ConfigSyncService.PushFirmwareAndModerationOnly с ЛЮБОЙ машины, не трогая остальной конфиг.
    ...
    private void EnsureColumnsExist()
    {
        ...
        // execution: ИСПОЛНЕНИЕ прошивки — короткая пометка вида «3 насоса» или «ПЧ Danfoss».
        //
        // У одного шкафа (тип + подтип + контроллер) бывает несколько прошивок, актуальных
        // ОДНОВРЕМЕННО: они отличаются не возрастом, а исполнением — в одной по умолчанию выбран один
        // тип частотников, в другой другое количество насосов. Пока такого признака не было, вторая
        // прошивка считалась просто более свежей версией первой: выдача схлопывала их в одну строку,
        // а NotSuperseded выкидывал предыдущую из «актуального» и из модерации.
        //
        // Пустое значение = обычная прошивка, поведение ровно как раньше: и схлопывание выдачи, и
        // «эту версию уже сменила более свежая», и метка «Текущая» для пустого исполнения работают в
        // точности как до появления столбца. Поэтому миграции данных здесь не нужно — ALTER TABLE с
        ...
        // всем, а отмена отката — нет, потому что правило звучало «активная может стать откатанной,
        // обратно никогда». Получалась дверь в одну сторону: вернул версию в строй, отправил
        // изменения — и первый же снимок машины, которая об отмене ещё не знает, откатывал её снова.
        // Ровно жалоба: «делаю актуальной новую, она откатывается сама».
        //
        // Пустое значение — снимок со старой версии программы: тогда поведение прежнее, монотонное.
        AddColumnsIfMissing("fw_versions", ("status_changed_at", "TEXT NOT NULL DEFAULT ''"));
    ...
    {
        // fw_versions и param_files здесь наравне со справочниками: у уже загруженных прошивок и
        // файлов параметров sync_id ещё нет, а без него импорт продолжал бы опознавать их только
        // натуральным ключом — то есть удаление не разъезжалось бы, а правка ключа (перенос версии
    ...
    /// Поправка: «кст текст не инструкция, а руководство по эксплуатации» — эту строку читает
