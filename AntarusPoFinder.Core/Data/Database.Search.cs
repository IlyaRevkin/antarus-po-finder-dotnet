...
public partial class Database
{
        ...
        double usageMultiplier = 1, bool showAllVersions = false)
    {
        ...
        //
        // Галка «показывать все версии» при ПУСТОМ запросе тоже осмысленна и означает ровно то, что
        // на ней написано: показать всё, что есть. Так её и просили — «при пустом поле поиска
        // должна показывать всё имеющееся». Без этого она молчала до первого введённого слова, то
        // есть в самом нужном случае (посмотреть, что вообще загружено) не работала вовсе.
        // Пустой запрос БЕЗ галки и без фильтров по-прежнему не выдаёт ничего: вываливать весь
        // справочник тому, кто просто открыл страницу, незачем.
        if (qTokens.Length == 0)
        {
            if (filters.IsEmpty && !showAllVersions) return new();
        ...
        double usageMultiplier, bool showAllVersions = false)
    {
        ...
        foreach (var entry in scored)
        {
            ...
            //
            // Схлопывание к одной строке на «подтип + контроллер» придумано для обычной линейки:
            // там свежая версия заменяет прежнюю, и показывать обе — мусор в выдаче. У ОПЦ смысл
            // ровно обратный: это разовая сборка под КОНКРЕТНЫЙ шкаф (серийный номер, номер заявки),
            // она ничего не заменяет и жить должна рядом. Общий ключ приводил к тому, что из двух
            // оставалась одна — отсюда сразу две жалобы: «ОПЦ перезаписала стандартную прошивку и
