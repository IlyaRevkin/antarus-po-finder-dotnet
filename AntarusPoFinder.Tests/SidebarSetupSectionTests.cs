...
public class SidebarSetupSectionTests
{
    ...
    /// <summary>«ДОПОЛНИТЕЛЬНО» — «заглянуть раз в месяц»: Настройки, Сетевые диски, Тикеты и чистка
    /// диска. На неё завязан живой GUI-стенд, и переезд наклеек с паспортами не должен был её
    /// задеть; «Чистка диска» добавлена сюда же и осознанно — она админская и редкая, в секцию «ДЛЯ
    /// НАЛАДЧИКА» ей нельзя (наладчику она не положена вовсе, см. RoleAccess).</summary>
    [Fact]
    public void TheOldMoreSection_IsUntouched()
    {
        ...
        Assert.Equal(new[] { "network", "tickets", "cleanup" },
