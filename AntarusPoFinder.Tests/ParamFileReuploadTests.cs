...
///
/// Просьба была такая: «старые файлы сохранять для просмотра, новые для открытия… просто дату
/// загрузки добавлять, всегда открывать свежую, а кому нужна старая — пусть откроет папку и найдёт.
/// И поле описание чтобы вело учёт изменений».
///
/// Прежняя оговорка «без полной истории версий» снята владельцем 25.08.2026. История теперь есть, но
/// не по самому .par (его никто не разбирает), а по таблице параметров рядом с ним — см.
/// Domain/ParamTable.cs. Раскладка файлов на диске от этого не изменилась.</summary>
